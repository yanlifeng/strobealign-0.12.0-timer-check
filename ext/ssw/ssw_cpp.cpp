...
#include <sys/time.h>
			...
			const Filter& filter, Alignment* alignment, const int32_t maskLen) const
	{
		...
		if(cnt == 0) {
			...
			if (!isDataMapReady) {
				...
				while (getline(file, line)) {
					...
					// 存储数据
					dataMap[{num1, num2}] = {v1, v2, v3, v4, v5};
				}
		...
		//  fprintf(stderr, "score_matrix_size : %d\n", score_matrix_size_);
		//  fprintf(stderr, "score_matrix : ");
		//  for(int i = 0; i < score_matrix_size_ * score_matrix_size_; i++) fprintf(stderr, "%d ", score_matrix_[i]);
		//  fprintf(stderr, "\n\n");
		//  fprintf(stderr, "%s\n%s\n", query, ref);

		//  fprintf(stderr, "translated_query : ");
		//  for(int i = 0; i < query_len; i++) fprintf(stderr, "%d ", translated_query[i]);
		//  fprintf(stderr, "\n");
		//  fprintf(stderr, "\n");

		//  fprintf(stderr, "translated_ref : ");
		//  for(int i = 0; i < ref_len; i++) fprintf(stderr, "%d ", translated_ref[i]);
		//  fprintf(stderr, "\n");
		//  fprintf(stderr, "\n");
		//}
		s_align* s_al; 
		...
		if(data.value1 + data.value2 + data.value3 + data.value4 + data.value5 == 0) {
			...
			s_al = my_ssw_align(profile, translated_ref, valid_ref_len,
					static_cast<int>(gap_opening_penalty_),
					static_cast<int>(gap_extending_penalty_),
					flag, filter.score_filter, filter.distance_filter, maskLen, data.value1, data.value2, data.value3, data.value4, data.value5);
		}
